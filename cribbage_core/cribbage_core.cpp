...
Targeting embedded systems, so uses small containers like card_t at the risk of
loss of speed on larger machines
...
void Cribbage::prep_score_hand(std::vector<card_t> &hand, card_t starter,
  ...
  if (!build_list) {
    ...
    // if we're building score lists, we need to build a mapping from sorted
    // back to original too THIS ASSUMES THAT THE VALUES STORED IN CARD_T AND
    // INDEX_T CAN COEXIST IN 32-BIT CONTAINERS! which is true. cards are only
    // ever 0..51 and, in this case, index is 0..4 so: build an array of card <<
    // 16 | index - want card in the upper half bc that's what we sort by
    std::array<uint32_t, 5> mapsort_temp;
    for (uint32_t j = 0; j < 5; j++)
      mapsort_temp[j] = uint32_t(whole_hand[j]) << 16 | j;
    std::sort(mapsort_temp.begin(), mapsort_temp.end());
    // copy cards over to sorthand_nranks
    std::transform(mapsort_temp.begin(), mapsort_temp.end(),
                   sorthand_nranks.begin(),
  ...
  // finally, boil sorted_ranks down to its ranks and subtract off first card's
  // rank for run spotting
  card_t first_rank = cu.rank(sorthand_nranks[0]);
  std::transform(sorthand_nranks.begin(), sorthand_nranks.end(),
                 sorthand_nranks.begin(),
                 [first_rank, this](card_t card) -> card_t {
                   return cu.rank(card) - first_rank;
                 });

  // then get whole_hand's suits? Not sure we need to do this here, could defer
  // until we know we need to check for flushes extract suits
  std::transform(whole_hand.begin(), whole_hand.end(), whole_suits.begin(),
                 [this](card_t card) -> card_t { return cu.suit(card); });
}

const int NUM_FIVECARDERS =
    11;  // bc for some reason I can't do .size() on the upcoming
...
const card_t fivecard_score_indices[NUM_FIVECARDERS] = {
    ...
    Cribbage::SCORE_TRIPLERUN,  //"triple run"        # 15 = 3*3 runs + 3*2
                                //pairs
    Cribbage::SCORE_TRIPLERUN,  //"triple run"
    Cribbage::SCORE_TRIPLERUN,  //"triple run"
    Cribbage::SCORE_DBLDBLRUN,  //"double double run" // 16 = 4*3 runs + 2*2
                                //pairs
...
const int NUM_FOURCARDERS =
    5;  // bc for some reason I can't do .size() on the upcoming
...
const card_t fourcard_score_indices[NUM_FOURCARDERS] = {
    ...
    Cribbage::SCORE_DBLRUN3, Cribbage::SCORE_DBLRUN3, Cribbage::SCORE_DBLRUN3};

// also scores hand + starter, but optimized for speed and doesn't worry about
// how scores would be read out scores is a pointer so can use nullptr when
// build_list is false and not need a dummy vector?
index_t Cribbage::score_shew(std::vector<card_t> hand, card_t starter,
                             std::vector<score_entry> *scores,
                             bool build_list) {
  // SHOULD THIS BE THE VERSION AND IT LOOKS JUST LIKE THE FAST ONE BUT WITH A
  // FLAG SAYING WHETHER TO BUILD THE LIST OR NOT AND LIST IS OF ALL THE
  // PRIMITIVES AND PATTERN MATCHING ON THE PRIMITIVES LIST CAN DERIVE THE NAMES
  // e.g. say you had primitives of pair, pair, pair you could remove those and
  // replace with a pair royal YES THAT DO IT LIKE THAT but first I'll ignore
  // build_list and scores precompute some useful views of the hand
  std::array<card_t, 5> whole_hand;
  std::array<card_t, 5> whole_vals;
  std::array<card_t, 5> sorthand_nranks;
  std::array<card_t, 5> whole_suits;
  std::array<index_t, 5> sort_map;
  prep_score_hand(hand, starter, whole_hand, whole_vals, sorthand_nranks,
                  whole_suits, sort_map, build_list);

  // figure out if we will be building a score list. build_list should only be
  // true if scores is non-nullptr, but be sure
  ...
  index_t totvals =
      index_t(std::accumulate(whole_vals.begin(), whole_vals.end(), 0));
  if (totvals == 15) {
    totscore += scorePoints[SCORE_FIFTEEN];
    // participating cards is all of them - so 0001 1111 bc we count from right,
    // first card = rightmost bit; therefore 0x1F
    if (make_list) scores->push_back(score_entry(0x1F, SCORE_FIFTEEN));
  }
  // if the total value is < 15, don't need to check for any fewer-card ones.
  // rare case - worth checking? Sure why not
  else if (totvals > 15) {
    // 5 choose 4 = 5 4-card sums, which can be considered the 5 card sum minus
    // each single card value
    for (i = 0; i < 5; i++)
      if (totvals - index_t(whole_vals[i]) == 15) {
        totscore += scorePoints[SCORE_FIFTEEN];
        // participating cards is all but i
        if (make_list)
          scores->push_back(score_entry(0x1F & ~(1 << i), SCORE_FIFTEEN));
      }
    // 5 choose 3 = 10 3-card sums (or total value minus 2 cards' values so only
    ...
    for (i = 0; i < 4; i++)
      for (j = i + 1; j < 5; j++)
        if (totvals - index_t(whole_vals[i] + whole_vals[j]) == 15) {
          ...
          if (make_list)
            scores->push_back(
                score_entry(0x1F & ~(1 << i | 1 << j), SCORE_FIFTEEN));
        }
    ...
    for (i = 0; i < 4; i++)
      for (j = i + 1; j < 5; j++)
        if (whole_vals[i] + whole_vals[j] == 15) {
          ...
          if (make_list)
            scores->push_back(score_entry((1 << i | 1 << j), SCORE_FIFTEEN));
        }
  }
  ...
  for (j = 0; j < NUM_FIVECARDERS; j++) {
    if (sorthand_nranks == fivecard_patterns[j]) {
      ...
      if (make_list)
        scores->push_back(
            score_entry(index_t(0x1F), fivecard_score_indices[j]));
      fivecard_found = true;
      break;
    }
  }
  ...
  if (!fivecard_found) {
    ...
    // so, mimicking the python version, it assumes that if there's a match in
    // the first 4, there won't be in the last 4. So we can defer building the
    // second four
    ...
    if (!fourcard_found) {
      // build second four
      std::transform(sorthand_nranks.begin() + 1, sorthand_nranks.end(),
                     last4.begin(), [firstlastrank](card_t rank) -> card_t {
                       return rank - firstlastrank;
                     });
      ...
      for (j = 0; j < NUM_FOURCARDERS; j++) {
        if (last4 == fourcard_patterns[j]) {
          ...
          if (make_list) {
            ...
            scores->push_back(
                score_entry(partcards, fourcard_score_indices[j]));
          }
          fourcard_found = true;
          break;
        }
      ...
      if (!fourcard_found) {
        ...
        re: nsrnks needing a sort ....won't they always be sorted? they do need
        normalizing. Is it the case that all we need to do is a sliding window
        of first 3, 2nd 3, 3rd 3? bc at this point we know that there aren't
        double or triple runs, which should be the only way there could be
        non-contiguous runs in the sorted ranks, yes? So let's try it that way


        for i in range(0, 3):
            for j in range(i + 1, 4):
                for k in range(j + 1, 5):
                    # how do we look for a run? get the sorted of the 3 cards'
        ranks. nsrnks =
        sorted([self.rank(cards[i]),self.rank(cards[j]),self.rank(cards[k])])
                    nsrnks = [x-min(nsrnks) for x in nsrnks]
                    if nsrnks == [0,1,2]:
                        curscore += 3
                        #print(cardstring(cards[i]),cardstring(cards[j]),cardstring(cards[k]),"...
        run of 3 -", #      curscore) score_subsets.append([[i,j,k],
        self.SCORE_RUN3])
        */
        // don't need to construct or normalize anything! just check for ranks
        // in order should also be exclusive bc 0,1,2 as first 3 and 1,2,3 as 2nd
        // would have been caught as a run of 4
        for (j = 0; j < 3; j++) {
          if ((sorthand_nranks[j] == sorthand_nranks[j + 1] - 1) &&
              (sorthand_nranks[j + 1] == sorthand_nranks[j + 2] - 1)) {
            totscore += scorePoints[SCORE_RUN3];
            // participating cards are j, j+1, j+2 = 7 << j, yes? no. This is
            // sorted hand
            if (make_list)
              scores->push_back(score_entry((1 << sort_map[j]) |
                                                (1 << sort_map[j + 1]) |
                                                (1 << sort_map[j + 2]),
                                            SCORE_RUN3));
            break;
        ...
        // pairs
        // so just compare every two cards' ranks and count a pair if they're
        // equal can it be short-circuited? think re: - yes, 4 of a kind is up
        // above as a 4 card pattern could I use a permutation operator for this?
        // Might be fun to try but I think this is more concise FOR BUILDING
        // SCORE LISTS, WILL NEED TO KEEP TRACK OF PAIR RANKS N STUFF? that can
        // all be postprocessing do count # pairs
        index_t numpairs = 0;
        for (i = 0; i < 4; i++)
          for (j = i + 1; j < 5; j++)
  ...
  // ************************************************************************************************************
  // ************************************************************************************************************
  // so is there a permutationy way to handle runs that will let us avoid the
  // sorting and normalization? worth considering but I think I'm prematurely
  // optimizing enough
  // ************************************************************************************************************
  // ************************************************************************************************************
  // ************************************************************************************************************

  // flushes - only counts if all 4 cards in the hand have the same suit. Then
  // if it's also the same suit as the starter (whole_hand[4])s, you get an extra
  // point
  if ((whole_suits[0] == whole_suits[1]) &&
      (whole_suits[0] == whole_suits[2]) &&
  ...
  // nobs: if starter is not a jack, and hand includes a jack with the same suit
  // as starter jack is rank 10
  if (cu.rank(starter) != 10) {
    for (j = 0; j < 4; j++) {
      if (whole_suits[j] == cu.suit(starter) && cu.rank(whole_hand[j]) == 10) {
        ...
        if (make_list)
          scores->push_back(score_entry(0x10 | (1 << j), SCORE_NOBS));
        break;
      }
    }
  }
...
// score_entrys for any scoring combinations made by the play. Score_entry is
...
index_t Cribbage::play_card(std::vector<card_t> &stack, card_t card,
  ...
  // so initial value is val of card, add on vals of stack, see if it's <= 31.
  // If so, legal!
  ...
  // debug
  /*
  for(auto j = 0; j < stack.size(); j++) printf("%s
  ",getCardUtils().cardstring(stack[j]).c_str()); printf("playing %s
  ",getCardUtils().cardstring(card).c_str()); printf("total value = %d\n",
  ...
  */
  ...
  // std::for_each(stack.end()-3, stack.end(), [this](card_t c){printf("%s
  // ",cu.cardstring(c).c_str());}); printf("\n");
  ...
  if (curtotal == 15) {
    ...
    if (make_list) {
      partcards = (1 << (stack.size() + 1)) -
                  1;  // e.g. if stack has 3 cards, (1<<(3+1))-1 = binary 111
  ...
  // so, the second card is stack[(stack.size()-1)-1], third
  // stack[(stack.size()-1)-2], fourth stack[(stack.size()-1)-3] as long as
  // those have the same rank as the new card, add 1 to # rank matches. If one
  // mismatches, stop
  card_t currank = cu.rank(card);
  index_t numrankmatch = 1;
  for (auto j = 1; j < 4 && j < rankstack.size(); j++)
    if (rankstack[(rankstack.size() - 1) - j] == currank)
      numrankmatch++;
    else
      break;
  // participating cards are at the low end of the bit field - so if there are,
  // e.g. , 4 cards participating, you want 00001111 binary, which is
  // (1<<(4+1))-1
  partcards = (1 << (numrankmatch + 1)) - 1;
  switch (numrankmatch) {
    // 1-4 should be the only possibilities, but still.
    case 2:
      total_score += scorePoints[SCORE_PAIR];
      if (make_list) scores->push_back(score_entry(partcards, SCORE_PAIR));
      break;
    case 3:
      total_score += scorePoints[SCORE_PAIRROYAL];
      if (make_list) scores->push_back(score_entry(partcards, SCORE_PAIRROYAL));
      break;
    case 4:
      total_score += scorePoints[SCORE_4KIND];
  ...
  // I think it was take max and min, and the diffie between them was #cards -1,
  // it's a straight, as long as there are no pairs.
  // ... that is what vpok did, but the writuep doesn't mention there not being
  // pairs! but the code does:
  ...
  // so, since we're counting from the end of the stack - we already know if
  // there were pairs, because we just found them. therefore, if there were any
  // pairs, there can't be a run. which means: for j = [3, stack.size())
  //    find min and max of last j cards in stack
  //      if((max-min) == j-1) longestrun = j;
  index_t longestrun = 0;

  // BUGFIXED: this wasn't quite right - pairs aren't the only thing that can
  // mess this up. cards of the same value separated by intervening cards can
  // also cause trouble, as with 3 2 A 4 5 6 A gets a run of 6!
  //...hm... so, non-contiguous pairs, which is the kind that were in the vpok
  //scorer, so it *was* true that pairs were all you had to worry about. Also,
  // the scope for both runs and pairs was the whole hand. how can you ensure
  // that no two cards' ranks are the same in a list? could set up an array of
  // the counts of ranks in the given range, yes? and if any are >1, the run at
  // that length is broken (therefore any longer ones are too).
  ...
  if (stack.size() > 2 && numrankmatch < 2) {
    ...
    for (index_t j = 3; j <= stack.size(); j++) {
      // find out if all the cards in the relevant subset are distinct. if any
      // two are equal, the range check doesn't guarantee a run.
      rankcounts.fill(0);
      for_each(rankstack.end() - j, rankstack.end(),
               [&rankcounts](card_t c) { rankcounts[c]++; });
