...
class Cribbage {
 ...
 public:
  ...
  class score_entry {
    ...
    */
    ...
    void set_card_involved(index_t index) { if (index < 5) index |= (1 <<
    index); } void clear_card_involved(index_t index) { if (index < 5) index &=
    ~(1<<index); }
    */
  };
 ...
 public:
  ...
  // * normalized i.e. subtract off lowest, ranks of the sorted whole hand (for
  // spotting runs)
  // * suits of the unsorted whole hand (for spotting flushes)
  // * mapping of sorted hand index to original hand index (if building score
  // lists)
  ...
  // and fills the scores vector with the score entries for whatever the hand
  // has in it, empty if nothing
