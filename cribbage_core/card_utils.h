...
card_utils.h - class definition for card game utilities - individual cards and
decks
...
// MASSIVELY IMPORTANT THING so that cards can be done as different types easily
// for performance fiddlement will use card_t for ranks and suits, too, bc I
// treat them like simplified cards
...
class CardUtils {
 public:
  ...
  // we're likely to be using it for tiny numbers but allow for up to 2
  // billion+, why not returns RANDOM_ERROR on illegal input
  ...
  */
  ...
  // card utilities
  // -------------------------------------------------------------- value to
  // return as a card if there is an error
  ...
  // cards are encoded as (rank * 4) + suit, where rank 0..12 = A234567890JQK
  // and suit 0..3 is heard, diamond, club, spade
  ...
  inline card_t val(card_t card) {
    return (rank(card) < 10) ? rank(card) + 1 : 10;
  }

  inline card_t suit(card_t card) { return card & 3; }

  // string versions are mostly for debug/test unless there's a text version of
  // the game
